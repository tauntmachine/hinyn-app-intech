...
const MainBox = styled(Box)`
    background: ${props => props.bg === 'full' ? 'linear-gradient(68deg, #EB4C60 0%, #79222D 100%)' :  '#EBEBEB'};
    width: 100%;
    height: auto;
    padding-bottom:5%;
`
  ...
  return (
    <MainBox bg={currentActiveForm === 2 ? 'full' : ''}>
        ...
        <Container sx={{paddingTop:'2rem'}}>
            <LogoBox> <Logo type="white"/> </LogoBox>
        </Container>
        <Container maxWidth="xl" sx={{position:'relative'}}>
            {/* <Box sx={{position:'absolute'}}> */}
                {/* { currentActiveForm === 1 ? <PostProjectForm1 handleNextClick={handleNextClick} /> : null } */}
                { currentActiveForm === 2 ? <ProjectSuccessPost handleNextClick={handleNextClick} /> : null }
            {/* </Box> */}
        </Container>
    </MainBox>
  )
