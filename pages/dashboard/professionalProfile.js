      ...
      <Container maxWidth="xl">
        <Grid container spacing={4} sx={{ padding: '5rem 0 7rem 0' }}>
          <Grid item xs={8}>
                    ...
                    <Row sx={{ gap: '8px', flexWrap: 'wrap' }}>
                      {clientCategories &&
                        clientCategories.map((category, idx) => {
                          return (
                            <>
                              <LargeIcon key={'user-category-' + idx}>
                                <PillWithIcon
                                  category={category?.attributes?.key}
                                  noPadding
                                  color="gray"
                                />
                              </LargeIcon>
                              {clientCategories.length > 1 &&
                              clientCategories.length - 1 !== idx ? (
                                <LeftBorder />
                              ) : null}
                            </>
                          );
            ...
            <VerticalDivider />
            {userData?.portfolios &&
              userData?.portfolios?.map((category, idx) => {
                return (
                  <>
                    <ContentBox
                      key={'portfolio-' + idx}
              ...
              <Row sx={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {skills &&
                  skills.map((skill, idx) => {
                    return (
                      <CustomStaticPill bg="green" key={'skill-' + idx}>
                        {' '}
                        {skill}{' '}
                      </CustomStaticPill>
                    );
