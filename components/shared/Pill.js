...
export const StaticPill = styled.div`
    background-color: ${props => props.bg === "green" ? "#ECF6F5" : "#ffffff"};
    border: ${props => props.color === "green" ? "1px solid #4AA398" : "none"};
    border-radius: 13px;
    display: flex;
...
const IconWrapper = styled(Box)`
    ...
    background-color: ${props => props.bg === "green" ? "#ECF6F5" : "#ffffff"};
    border: ${props => props.color === "green" ? "1px solid #4AA398" : "none"};
    color: ${props => props.color === "green" ? "#4AA398" : (props.color === 'gray' ? '#BABABA' : "#525252")};
`
...
export const PillWithIcon = ({color,bg,category,noPadding}) => {
  ...
  return (
        <IconWrapper color={color} bg={bg} noPadding={noPadding}>
            <Icon>
                <Image src={iconPath+data?.icon} alt="hinyn logo" layout="fill" class="icon-img"/>
            </Icon>
            {data?.title}
         </IconWrapper>
  )
